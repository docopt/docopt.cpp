...
namespace docopt {
	...
	public:
		...
		std::vector<Pattern*> flat(bool (*filter)(Pattern const*)) override {
			if (filter(this)) {
				return { this };
			}
			return {};
	...
	: public LeafPattern {
	public:
		using LeafPattern::&const LeafPattern;
	...
	public:
		using BranchPattern::&const BranchPattern;
...
#pragma mark inline implementations
	...
	static inline std::vector<PatternList> transform(PatternList pattern)
	{
		...
		while(!groups.empty()) {
			...
			if (auto* either = dynamic_cast<Either*>(child.get())) {
			...
			} else if (auto* oneOrMore = dynamic_cast<OneOrMore*>(child.get())) {
				...
				auto* branch = dynamic_cast<BranchPattern*>(child.get());
	...
	inline void BranchPattern::fix_repeating_arguments()
	{
		...
		for(auto const& group : either) {
			...
			for(auto const& e : group_set) {
				...
				auto* leaf = dynamic_cast<LeafPattern*>(e.get());
				if (!leaf) continue;
				...
				if (dynamic_cast<Command*>(leaf)) {
					...
					ensureList = true;
				} else if (auto* o = dynamic_cast<Option*>(leaf)) {
					if (o->argCount()) {
						ensureList = true;
					} else {
						ensureInt = true;
	...
	inline Option Option::parse(std::string const& option_description)
	{
		...
		if (argcount) {
			...
			if (std::regex_search(options_end, option_description.end(),
						  match,
						  std::regex{R"(\[default: (.*)\])", std::regex::icase}))
			{
				val = match[1].str();
			}
		}
